...
class GemmaAttention(nn.Module):
    ...
    def __init__(self, config: GemmaConfig, layer_idx: Optional[int] = None):
        ...
        self.ablated_attention_heads = None
        print(f'Initializing GemmaAttention with Index: {layer_idx}, Ablated Heads: {self.ablated_attention_heads}')
    ...
    def ablate_attention_heads(self, ablated_attention_heads):
        if ablated_attention_heads is not None and self.layer_idx in ablated_attention_heads:
            self.ablated_attention_heads = ablated_attention_heads[self.layer_idx]
        else:
            self.ablated_attention_heads = None
    ...
    def forward(
        ...
        **kwargs,
        ...
        intermediate_values = {}  # Dictionary to store intermediate values

        bsz, q_len, _ = hidden_states.size()
        intermediate_values["input"] = hidden_states
        # intermediate_values["hidden_states"] = hidden_states
        # intermediate_values["bsz"] = bsz
        # intermediate_values["q_len"] = q_len

        query_states = self.q_proj(hidden_states)
        key_states = self.k_proj(hidden_states)
        value_states = self.v_proj(hidden_states)
        # intermediate_values['query_states_projected'] = query_states
        # intermediate_values['key_states_projected'] = key_states
        # intermediate_values['value_states_projected'] = value_states

        query_states = query_states.view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
        key_states = key_states.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
        value_states = value_states.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
        # intermediate_values['query_states_transposed'] = query_states
        # intermediate_values['key_states_transposed'] = key_states
        # intermediate_values['value_states_transposed'] = value_states


        past_key_value = getattr(self, "past_key_value", past_key_value)
        cos, sin = self.rotary_emb(value_states, position_ids, seq_len=None)
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, None)
        intermediate_values['projected_Q'] = query_states
        # intermediate_values['query_states_rotary'] = query_states
        # intermediate_values['key_states_rotary'] = key_states
        ...
        intermediate_values['projected_K'] = key_states
        intermediate_values['projected_V'] = value_states
        # intermediate_values['key_states_repeated'] = key_states
        # intermediate_values['value_states_repeated'] = value_states
        ...
        intermediate_values['attention_weights'] = attn_weights
        ...
        # intermediate_values['attn_output_pre_proj'] = attn_output
        ...
        intermediate_values['output'] = attn_output
        # intermediate_values['final_attn_output'] = attn_output

        if not output_attentions:
            attn_weights = None
        # print("In GemmaAttention")

        return attn_output, intermediate_values, past_key_value
...
class GemmaDecoderLayer(nn.Module):
    ...
    def ablate_attention_heads(self, ablated_attention_heads):
        self.self_attn.ablate_attention_heads(ablated_attention_heads)
...
class GemmaModel(GemmaPreTrainedModel):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        ablated_attention_heads: Optional[dict] = None,
        ...
        if ablated_attention_heads is not None:
            for decoder_layer in self.layers:
                decoder_layer.ablate_attention_heads(ablated_attention_heads)
...
class GemmaForCausalLM(GemmaPreTrainedModel):
    ...
    def set_ablated_heads(self, ablated_attention_heads):
        # Given a dictionary of attention heads, ablate the attention heads in the model
        # The dictionary should be of the form {layer_num: [head_num1, head_num2, ...]}

        for layer in self.model.layers:
            layer.ablate_attention_heads(ablated_attention_heads)
    
    def reset_ablated_heads(self):
        # Reset the ablated attention heads in the model
        for layer in self.model.layers:
            layer.ablate_attention_heads({})
    
    def print_ablated_heads(self):
        # Print the ablated attention heads in the model ablated attention heads
        ablated = False 
        for layer in self.model.layers:
            ablated_heads = layer.self_attn.ablated_attention_heads
            if ablated_heads:
                print(f"Layer {layer.self_attn.layer_idx}: {ablated_heads}")
                ablated = True
        if not ablated:
            print("No attention heads have been ablated")
    ...
    def forward(
        ...
        # print("Hello from GemmaForCausalLM")
