...
def solve_prompts(prompts_dict, model, tokenizer, device, num_tokens_to_generate=128,temperature=0.7, attention_measure=constants.ATTENTION_MEASURE):
    ...
    for category in list(prompts_dict.keys()):
        ...
        all_attention_weights[category] = generate_and_analyze_prompts(prompts_dict[category], model, tokenizer, device, num_tokens_to_generate, 
...
def plot_categories_comparison(all_attention_weights, save=False, base_plot_path=None, split_half=False, split_third=False):
    ...
    if split_half:
        ...
        half_layers = num_layers // 2 # all_attention_weights, save=True, base_plot_path=None, layer_indices=None, filename='head_activations_comparison
        ...
        stats_dict = plot_attention_weights_comparison(all_attention_weights, save=save, base_plot_path=base_plot_path, 
            layer_indices=(half_layers+1, num_layers), filename='head_activations_comparison_2')
    elif split_third:
        num_layers = len(all_attention_weights[list(all_attention_weights.keys())[0]])
        ...
        stats_dict = plot_attention_weights_comparison(all_attention_weights, save=save, base_plot_path=base_plot_path, 
            layer_indices=(2*third_layers + 1, num_layers), filename='head_activations_comparison_3')
    else:
        stats_dict = plot_attention_weights_comparison(all_attention_weights, save=save, base_plot_path=base_plot_path)
